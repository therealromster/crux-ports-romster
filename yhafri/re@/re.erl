...
nfa_bin_s(Bin, P, Nfa, Ss0, A) ->
	...
	<<_:P1/binary,C,_/binary>> ->
		...
		{Ss1,none} ->
		    nfa_bin(Bin, P+1, Nfa, Ss1, A);
		{Ss1,Subs} ->
		    nfa_bin(Bin, P+1, Nfa, Ss1, {match,Subs,P})
	    end;
	_ ->					%No more characters.
...
has_match_s(P, [{S,Subs}|Ss], Nfa) ->
    case element(S, Nfa) of
	#cstate{c=eos,s=S1} ->			%EOS is only valid here
	    Ss1 = eclosure_s(S1, Nfa, [], Ss, P, Subs),
	    has_match_s(P, Ss1, Nfa);
	#cstate{c=done} ->
	    case has_match_s(P, Ss, Nfa) of
		{yes,Subs1}=Yes when Subs1 > Subs -> Yes;
		_ -> {yes,Subs}
	    end;
	_ -> has_match_s(P, Ss, Nfa)
    end;
has_match_s(_, [], _) -> no.
